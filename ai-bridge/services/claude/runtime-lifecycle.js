...
export function startPerpetualReader(runtime, callbacks) {
   ...
   */
  const emitSessionScopedEvent = (event, sessionId, extra = {}) => {
    try {
      ...
      const eventPayload = { type: 'daemon', event, sessionId, ...extra };
  ...
  // Refresh the chat window for a session (re-reads persisted JSONL on the Java side).
  const emitInterTurnEvent = (sessionId) => emitSessionScopedEvent('session_updated', sessionId);
  ...
  const emitInterTurnActivity = (sessionId, active) =>
    emitSessionScopedEvent('inter_turn_activity', sessionId, { active });
