...
// These three spawn an absolute POSIX shell (/bin/bash). Node on Windows cannot
// resolve that path even under Git Bash, so they are POSIX-only. CI runs the
// ai-bridge suite on ubuntu-latest, where they still execute.
const posixShellOnly = { skip: process.platform === 'win32' ? 'requires a POSIX /bin/bash' : false };
...
test('create with Grok-style /bin/bash -lc args', posixShellOnly, async () => {
  const host = new AcpTerminalHost({
    authorizeCreate: async () => true,
...
test('shell string command with empty args', posixShellOnly, async () => {
  const host = new AcpTerminalHost({
    authorizeCreate: async () => true,
    env: { ...process.env, SHELL: '/bin/bash' },
  });
...
test('shell string with metacharacters runs as script not as binary name', posixShellOnly, async () => {
  const host = new AcpTerminalHost({
    authorizeCreate: async () => true,
    env: { ...process.env, SHELL: '/bin/bash' },
  });
