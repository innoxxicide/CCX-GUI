...
🐛 Fixes
- Fix **a permission prompt sometimes never appearing, leaving the agent stuck as though the command were still running**. A tool that needs approval (`Bash`, `Edit`, `Write`) blocks the Node bridge on a response file while the IDE shows the dialog, so anything that loses the request between the two hangs the turn with no way out — and with "Auto-close dialog on timeout" switched off there is no safety net on either side, so it hangs indefinitely rather than for the ~6 minutes the timeout would otherwise cost. Four independent paths could lose it. `CefBrowser.executeJavaScript` is fire-and-forget: while the JCEF renderer is booting, navigating, or recovering from a restart it drops the snippet with no exception and no return value, and the retry loop embedded inside that snippet cannot help because the snippet itself never ran — the webview now acknowledges every dialog it receives and the backend re-sends an unacknowledged one up to five times. In the webview, an effect mirrored dialog state back onto the refs that decide whether an incoming request is shown or queued, and it ran one commit behind: a request arriving between a commit and its effect flush had its refs overwritten and a queued request opened on top of it, leaving it neither showing nor queued while the backend still waited on its answer — the refs are now the only source of truth and every close path drains the queue synchronously. The IPC watcher thread exited permanently on any interrupt, and an `Error` escaping a request handler killed it outright, both while leaving its "running" flag set — so it could never be restarted and every later request in that session went unread. And two paths consumed the request file without writing a response: an exception after the file had been deleted, and the 30-second timeout of the system-dialog fallback, which also left the file in place for the watcher to re-dispatch on every poll. Every path that consumes a request now answers it, defaulting to deny
...
🔄 Upstream sync
- **Merged upstream CC GUI v0.5.3** (`zhukunpenglinyutong/jetbrains-cc-gui`), covering the whole v0.5.0 → v0.5.3 run. Fork identity (plugin id, `CCXG` tool window, `CCXGUI.` action namespace, green `ccx-gui-icon.svg`, `com.github.ccxgui` package) and every fork-only feature — Claude usage battery, agent error/question notification sounds, Concise mode, permission-dialog timeout toggle, auto-resume after a usage-limit reset, Keep computer awake, Send scheduled, Send now, Retry automatically after an error — are preserved. What arrives with it: **five new CLI providers** (DeepSeek Harness, Grok, Kimi, OpenCode, Pi) behind a shared marker-stream bridge, with their own settings panels, model lists, permission modes and session history; **compact transcript restoration**, so reopening a compacted conversation restores the full pre-compaction history instead of just the summary; a **multi-tab webview rendering fix** so open tabs keep their content and repaint independently; a **JCEF out-of-process NPE workaround** (JBR-9234) offered as a one-click restart; terminal context-menu actions registered dynamically so they no longer crash IDE 2023.3 at startup; a **batched settings bootstrap** that stops opening Settings from stampeding the browser process, with provider/agent/prompt lists now fetched when their tab first mounts; and per-session **net file-change stats** with multi-agent marks
- **Retire the dead `claude-sonnet-4-7` default.** Upstream's migration maps retired model ids (`sonnet-4-6`, `sonnet-4-7`, `opus-4-6`) to their live replacements on write, so a tab or history session persisted against a since-retired model self-heals instead of spawning a CLI pinned to a model the API rejects on every send. This fork's `SessionState` default moved to `claude-sonnet-5` to match, since it was still handing out one of the retired ids
- **Take upstream's streaming ordering fix in place of this fork's force-push workaround.** Frames are now dropped only when genuinely out of order (older than the last frame actually pushed) rather than whenever newer data was merely queued — which had been discarding structural `tool_use` / `tool_result` frames throughout dense streaming. Because the new rule never advances the update sequence on the push path, the "API error never renders, then shows up on the next update" failure the fork's own fix targeted cannot recur, and that workaround has been removed rather than left stacked on top
- **Reconcile the two AskUserQuestion sound implementations.** Upstream added its own question-alert toggle while this fork already shipped one with a dedicated sound picker; the fork's control remains the single switch in Settings and stays wired to the reminder, so there is no second toggle that silently does nothing. Upstream's injectable notifier seam is kept, and its new only-when-unfocused gate for system notifications now applies to this fork's agent-error toast as well
- **Keep both provisional-state guards on the shared model snapshot.** `localStorage` is shared by every tab in the JCEF process, so the save pass now skips both the mount commit (which still sees pre-hydration props) and any window where the runtime page context or recovery state has yet to be established
...
- Add **Send now**: while an agent is working, a button beside the stop button sends what is in the input box straight away instead of queueing it behind the running turn — for when you can see the agent heading the wrong way and want to correct it without waiting the turn out. It stops the running turn and sends, because that is the only thing that actually intervenes. Handing a message to a live turn was measured against Claude Code CLI 2.1.220 first: a user message pushed into the running query's own input stream is queued as a turn of its own rather than folded into the current one, so it reaches the agent no sooner than the ordinary queue already gets it there; marking it `priority:"now"` does not change that, and on a message the CLI has not stamped with a uuid it ends the running turn outright with `error_during_execution`. None of that machinery ships, because none of it buys anything. The message is placed at the head of the existing queue and the turn is then interrupted, rather than the sender being called directly: the queue already drains on the loading edge, so routing through it is what keeps the stop and the send from racing, and anything already queued keeps its order behind the new message. The button reuses the ordinary submit path, so validation, input history, clearing and attachments behave exactly as on a normal send; it appears only while a turn is running and only becomes active once there is something to send. Works for Claude and Codex alike

- Add **Retry automatically after an error** (Basic configuration → Behavior, off by default, next to "Keep computer awake"). When a turn dies on an error, the agent is sent a short message to pick the work back up — three times a minute apart, then every five minutes until it answers. The message is editable in the same panel and defaults to "Continue working on the task."; a banner above the chat input shows the run in progress and stops it on one click. One recovery run per outage is the rule that makes this safe to leave on: once engaged, further errors advance the existing schedule instead of restarting it, so an API error answered by another API error cannot pin the retries to the one-minute cadence. The run ends the moment any turn finishes without an error — a message you sent by hand counts just as much as a retry does — and also ends if you turn the toggle off mid-run. Usage-limit stops are deliberately excluded and left to the auto-resume feature, which waits for the reset rather than hammering a blocked account; a limit reached mid-run stands the retries down and hands the session over. Works for Claude and Codex alike, and in multi-agent runs: a background agent's wake-up turn runs outside the normal request path, so its failures are reported separately and drive the same recovery. Like an armed auto-resume wake, a pending retry holds the machine awake — a nudge scheduled five minutes out does not fire if the machine idle-slept through it. Nothing is persisted across an IDE restart: a recovery run reacts to a live failure, and replaying one into a session you have walked away from would start work behind your back

🐛 Fixes
- Fix **a usage-limit stop not being registered as an error, so auto-resume never armed** — most visibly in multi-agent runs, where the limit notice appeared in the chat as ordinary agent text with no red error card, and the scheduled wake-up after the limit reset never fired. Claude Code does not fail a turn when the account runs out of quota: it ends the turn with a *synthetic* assistant message (`model: "<synthetic>"`, `error: "rate_limit"`, `stop_reason: "stop_sequence"`) carrying only the notice, and returns an ordinary success result. That message is text-only, so the streaming path suppressed its `[MESSAGE]` envelope and the error marker never reached the backend, while its text still streamed into the chat bubble; no `[SEND_ERROR]` meant no error card and no turn-error signal, which is the only thing that arms auto-resume. Two more paths carried the same stop and were dropped entirely: a subagent dying first, relayed to its parent as a Task `tool_result` ("Agent terminated early due to an API error: …") that the sidechain filter discarded before anything looked at it, and — the common case with background agents — a `task-notification` continuation, which the CLI runs outside the normal turn path where nothing could report it. All of them are now recognized in one place and routed to the standard error path, so a limit stop registers on the main agent no matter which agent hit it first. Auto-resume also no longer depends on the account-usage endpoint agreeing: that endpoint lags enforcement and returns nothing at all for API-key accounts, so the SDK's own verdict now arms the wake on its own authority, at the later of the two reset times, and survives a failed usage fetch. The reset time is taken from the SDK when it states one as a timestamp, otherwise read from the notice itself ("resets 3pm (Europe/Kiev)"); wake-time verification is unchanged, so an over-eager arm costs one re-check rather than a premature resume. Detection deliberately requires a structural marker and not just the notice text, so a pasted log — or the agent discussing a limit — cannot fabricate an error. Reloading a session now also restores such a stop as an error rather than as something the agent chose to say
- Fix the **context-window gauge dropping to 0% on every usage-limit stop**: the synthetic message's all-zero usage block was being reported as the turn's authoritative token count
- Fix **an "Invalid escape sequence" parser error surfacing when a session's history is loaded** — a `com.google.gson.stream.MalformedJsonException` naming a JSON path deep inside the conversation, which read as though the session file itself were damaged. It was not. Loading history spawns the Node bridge and parses its stdout, but the process was started with stderr merged into that same pipe, and the bridge logs its startup diagnostics there with raw Windows paths (`Permission dir: C:\Users\…`). The history payload is a single line of several megabytes, so a diagnostic write landing inside it leaves `\U` in the middle of a JSON string — an invalid escape, reported against whichever conversation field was being read at that offset, most often a `Bash` result's `stdout` because those are the longest strings in a session. The two streams now have separate pipes, so a log line cannot reach the payload at all; stderr is drained on its own thread, since an undrained pipe fills at ~64KB and would stall the bridge mid-payload. The payload is also now validated as a complete JSON document before being parsed, replacing a last-resort "take everything from the first brace" branch that could do nothing but splice trailing log lines into the data, and a read that still comes back unusable is retried once instead of being surfaced. The message used to outlive the failure as well: a load error was never cleared, so it was re-raised as a toast on every later state change — hence the same error appearing two and three times over
...
- Add a **"Keep computer awake while working" toggle** (Basic configuration → Behavior, off by default). While an agent is working — and while a run is waiting to restart after a usage limit resets — the machine is kept from idle-sleeping, via `SetThreadExecutionState` on Windows, `caffeinate -i` on macOS and `systemd-inhibit --what=idle:sleep` on Linux; unsupported platforms fall back to a no-op. Holding through the limit wait is the point of the feature, not a side effect: a scheduled auto-resume does not fire on time if the machine slept through its deadline, so the hold is handed straight from the failing turn to the armed wake, across the usage assessment in between, and is only dropped once the wake fires or auto-resume gives up. Holds are counted per window and released on tab close, so no closed tab can strand one. Only automatic idle sleep is suppressed — closing the lid, sleeping the machine yourself, and the display switching off all still work
...
🐛 Fixes
- Fix **a newly opened tab sometimes resetting the model and its reasoning effort to a default** instead of carrying over the selection in use. The backend hands each tab its own provider/model (`__INITIAL_TAB_PROVIDER__` / `__INITIAL_TAB_MODEL__`) so multi-tab setups do not clobber each other, and the webview treats that as an authoritative per-tab preference outranking its own remembered choice — but `SessionState` initializes both fields to hardcoded placeholders, so a session nobody had configured yet reported `claude` / `claude-sonnet-4-7` with the same confidence as a real selection and the webview discarded the user's pick. Hence the intermittency: a new tab inherits the tab it was opened from, which is right only while that tab's Java session already holds a real model — not when the webview's boot sync had yet to land, or when creating a session or loading history dropped it. Reasoning effort followed, because the level selector clamps to a supported value when the model changes under it and persists the clamped one, so a single substituted model corrupted the stored effort globally; effort was also missing from the per-tab handoff entirely. Provider and model are now marked as explicitly chosen or not (the flag travels with the value through `copyPreferencesFrom`, so an inherited placeholder stays a placeholder and is skipped when persisting tab state), reasoning effort joins the handoff as `__INITIAL_TAB_REASONING_EFFORT__` and survives a new session or a history load, and the webview's localStorage save no longer writes slice defaults on the mount pass — that snapshot is shared by every tab in the JCEF process, so a tab booting at that moment hydrated from them
- Fix **API errors and other turn failures not showing in the chat** — the red error panel stayed missing, and sometimes surfaced much later on an unrelated refresh. A failed turn appends its ERROR message to the session state and only then signals stream-end, so the stream-end flush carries a snapshot taken *before* the error and the error itself travels in a later, higher-sequence frame. Since the v0.4.8 sync the coalescer force-pushes that stale flush snapshot with a bumped sequence, which either overwrote the error bubble the webview had already rendered, or pushed `updateSequence` past a still-in-flight error frame so the webview's `__minAcceptedUpdateSequence` barrier rejected it outright. Long conversations hit it more often: a bigger payload spends longer being serialized, so the flush frame is more likely to land last. The force-push now stands down when a newer frame has already begun dispatch — keeping the upstream fix's intent of never losing a turn's tail without letting it overwrite newer state — and on the webview side an error tail now survives a shorter snapshot for every provider (deduped by text), so no late frame can erase an error already on screen
- Fix **a prompt sent while subagents are running disappearing from the transcript** (the agent still received and answered it). The optimistically rendered bubble was only carried across backend snapshots while it was the last entry in the list, but `onStreamStart` appends a streaming assistant placeholder the moment the answering turn opens — so any snapshot generated before the prompt was persisted dropped it, and it only came back once the backend's own copy landed, by then far above everything generated since. The unconfirmed bubble is now found anywhere in the tail (within a bounded window) and restored above the turn's assistant bubble rather than below the answer it triggered
- Fix the **Tasks and Subagents tabs going blank after a stopped run is resumed**. Both lists are derived from a message scope that narrows to the current turn while streaming, and resuming opens a new turn — so as soon as the resumed agent made its first tool call, the plan and the subagents left behind in the interrupted turn fell out of scope, including background agents that were still running. A turn boundary is no longer treated as a work-lifecycle boundary: the scope now reaches back to the turn that owns a still-running subagent (keeping its finished siblings, so a group reads `3/5` rather than `0/2`), and an unfinished plan is carried into the resumed turn. A fully completed plan is still not carried into a new request
...
🚀 Fork
- **CCX GUI (Claude or Codex) is a fork of CC GUI at v0.4.7** — original repository: https://github.com/zhukunpenglinyutong/jetbrains-cc-gui. Versioning restarts at v0.0.1 (by @innoxxicide)
- Rename the product to **CCX GUI**, with its own plugin id, Java package (`com.github.ccxgui`) and a distinct namespace for every registration id, so the fork can coexist with the original plugin in one IDE
- Recolor the brand mark from terracotta to green
