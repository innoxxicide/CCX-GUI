package com.github.ccxgui.provider.pricing;

import com.github.ccxgui.provider.CustomPricingProvider;
...
public final class ClaudePricingTable {
    ...
    // Order matters: normalize() takes the FIRST prefix the model id starts with, so a
    // longer id must precede the shorter one it extends (claude-fable-5-1 before
    // claude-fable-5), or the newer model silently inherits the older one's pricing row.
