...
- Fix **Concise mode leaking the plugin's own prompt sections into every non-Claude provider**. Concise mode promises that the plugin contributes nothing beyond the user's own message, but the gate was only implemented in the Claude path — Codex and the marker CLI providers (Grok, Kimi, OpenCode, Pi, DeepSeek Harness) assemble their prompt on the Java side, which never read the setting. With the toggle on they still received, every turn: the \`## Workspace Context\` / \`## Project Modules\` block, \`## Active Terminal Session\`, \`## Referenced Files\`, \`## User's Current IDE Context\` and the \`## Agent Role and Instructions\` wrapper. Codex additionally had the project's own \`AGENTS.md\` re-injected as an \`<agents-instructions>\` block — which Codex discovers by itself — and Grok had \`~/.grok/grok-rules.md\` and a JSON dump of the open editors appended. All of it is now suppressed when Concise mode is on, so these providers see what Claude sees: the message as typed. Attachments and images are unaffected, being the user's own input`,
  ...
  },
  {
    ...
    content: {
      en: `🔄 Upstream sync
- **Merged upstream CC GUI v0.5.3** (\`zhukunpenglinyutong/jetbrains-cc-gui\`), covering the whole v0.5.0 → v0.5.3 run. Fork identity and every fork-only feature — Claude usage battery, agent error/question notification sounds, Concise mode, permission-dialog timeout toggle, auto-resume after a usage-limit reset, Keep computer awake, Send scheduled, Send now, Retry automatically after an error — are preserved. What arrives with it: **five new CLI providers** (DeepSeek Harness, Grok, Kimi, OpenCode, Pi) behind a shared marker-stream bridge, with their own settings panels, model lists, permission modes and session history; **compact transcript restoration**, so reopening a compacted conversation restores the full pre-compaction history instead of just the summary; a **multi-tab webview rendering fix** so open tabs keep their content and repaint independently; a **JCEF out-of-process NPE workaround** (JBR-9234) offered as a one-click restart; terminal context-menu actions registered dynamically so they no longer crash IDE 2023.3 at startup; a **batched settings bootstrap** that stops opening Settings from stampeding the browser process; and per-session **net file-change stats** with multi-agent marks
- **Retire the dead \`claude-sonnet-4-7\` default.** Retired model ids (\`sonnet-4-6\`, \`sonnet-4-7\`, \`opus-4-6\`) now migrate to their live replacements on write, so a tab or history session persisted against a since-retired model self-heals instead of spawning a CLI pinned to a model the API rejects on every send
- **Take upstream's streaming ordering fix in place of this fork's force-push workaround.** Frames are dropped only when genuinely out of order rather than whenever newer data was merely queued — which had been discarding structural tool blocks throughout dense streaming. The new rule never advances the update sequence on the push path, so the "API error never renders, then shows up on the next update" failure the fork's own fix targeted cannot recur
- **Reconcile the two AskUserQuestion sound implementations.** Upstream added its own question-alert toggle while this fork already shipped one with a dedicated sound picker; the fork's control remains the single switch in Settings, so there is no second toggle that silently does nothing. Upstream's only-when-unfocused gate for system notifications now applies to this fork's agent-error toast as well

✨ Features
- Add **Send scheduled**: a clock button left of the send button opens a small date/time picker, pre-filled with the current date and time, and queues the message currently in the input box for delivery at the chosen moment. The input clears as it does on a normal send, and a strip above it shows what is queued and when, with a one-click cancel. The timer lives in the IDE rather than the webview, alongside the auto-resume-after-usage-limit wake and under the same rules: the pending message is persisted per tab, so it survives a webview reload, a tab switch and an IDE restart; the machine is held awake until it fires, since a scheduled wake does not fire on time if the machine idle-slept through its deadline; a restart across the deadline still delivers within 30 minutes, and anything staler is offered as a "Send now" prompt rather than started behind your back. The message must be non-empty, at most 10,000 characters, in the future and within 7 days — a rejected request says which rule it broke instead of quietly moving the time. One schedule per tab. If a turn is still streaming when the time comes, the send is deferred rather than interleaved, retried for up to ten minutes, then handed back for a manual send so the text is never lost. Works for Claude and Codex alike; attachments are not carried, so only the text is scheduled

- Add **Send now**: while an agent is working, a button beside the stop button sends what is in the input box straight away instead of queueing it — for when you can see the agent heading the wrong way and want to correct it without waiting out the turn. It stops the running turn and sends, which is the only way to actually intervene: a message handed to a live turn is queued as a turn of its own rather than folded into the running one, so it would reach the agent no sooner than the ordinary queue already gets it there. The message goes to the head of the queue, so the stop and the send cannot race and anything already queued keeps its order behind it. The button appears only while a turn is running, and only becomes active once there is something to send. Works for Claude and Codex alike

- Add **Retry automatically after an error** (Basic configuration → Behavior, off by default, next to "Keep computer awake"). When a turn dies on an error, the agent is sent a short message to pick the work back up — three times a minute apart, then every five minutes until it answers. The message is editable in the same panel and defaults to "Continue working on the task."; a banner above the chat input shows the run and stops it on one click. One recovery run per outage: once engaged, further errors advance the existing schedule instead of restarting it, so an API error answered by another API error cannot pin the retries to the one-minute cadence. The run ends the moment any turn finishes without an error — a message you sent by hand counts too — and also ends if you turn the toggle off mid-run. Usage-limit stops are excluded and left to auto-resume, which waits for the reset rather than hammering a blocked account; a limit reached mid-run hands the session over. Works for Claude and Codex alike, and in multi-agent runs, where a background agent's wake-up turn runs outside the normal request path. A pending retry holds the machine awake, since a nudge scheduled five minutes out does not fire if the machine idle-slept through it. Not persisted across an IDE restart, so a run is never replayed into a session you have walked away from

🐛 Fixes
- Fix **a usage-limit stop not being registered as an error, so auto-resume never armed** — most visibly in multi-agent runs, where the limit notice appeared in the chat as ordinary agent text with no red error card, and the scheduled wake-up after the reset never fired. Claude Code does not fail a turn when the account runs out of quota: it ends it with a *synthetic* assistant message carrying only the notice and returns an ordinary success result. Being text-only, that message was suppressed on the streaming path, so the error marker never reached the backend while its text still streamed into the chat bubble — and with no error reported, nothing armed auto-resume. Two more paths carried the same stop and were dropped entirely: a subagent dying first, relayed to its parent as a Task tool result the sidechain filter discarded, and — the common case with background agents — a task-notification continuation, which runs outside the normal turn path. All of them are now recognized in one place and routed to the standard error path, so a limit stop registers on the main agent no matter which agent hit it first. Auto-resume no longer depends on the account-usage endpoint agreeing either: it lags enforcement and returns nothing for API-key accounts, so the SDK's own verdict now arms the wake on its own authority, at the later of the two reset times. Reloading a session restores such a stop as an error rather than as something the agent chose to say
- Fix the **context-window gauge dropping to 0% on every usage-limit stop**
- Fix **an "Invalid escape sequence" parser error surfacing when a session's history is loaded**, naming a JSON path deep inside the conversation as though the session file itself were damaged. It was not. Loading history spawns the Node bridge and parses its stdout, but the process ran with stderr merged into that same pipe, and the bridge logs its startup diagnostics there with raw Windows paths (\`C:\\Users\\…\`). The history payload is a single line of several megabytes, so a diagnostic write landing inside it leaves \`\\U\` in the middle of a JSON string — an invalid escape, reported against whichever field was being read at that offset, usually a Bash result's \`stdout\` since those are a session's longest strings. The two streams now have separate pipes, the payload is validated as a complete JSON document instead of falling back to "everything from the first brace", and an unusable read is retried once. The error no longer lingers either: it was never cleared, so it was re-raised as a toast on every later state change`,
      zh: ``,
    },
  },
  {
...
- Add a **"Keep computer awake while working" toggle** (Basic configuration → Behavior, off by default). While an agent is working — and while a run is waiting to restart after a usage limit resets — the machine is kept from idle-sleeping, via \`SetThreadExecutionState\` on Windows, \`caffeinate\` on macOS and \`systemd-inhibit\` on Linux. Holding through the limit wait is the point: a scheduled auto-resume does not fire on time if the machine sleeps through its deadline. Only automatic idle sleep is suppressed — closing the lid or sleeping the machine yourself still works, and the display is left free to switch off
...
🐛 Fixes
- Fix **a newly opened tab sometimes resetting the model and its reasoning effort to a default** instead of carrying over the selection in use. The backend hands each tab its own provider/model so multi-tab setups don't clobber each other, and the webview lets that outrank its remembered choice — but a session nobody had configured yet still reported its placeholder defaults, indistinguishable from a real preference. The effort followed, because the level selector clamps to a supported value when the model changes under it and saves the clamped one. Reasoning effort is now part of the per-tab handoff as well, and survives creating a new session or loading history
- Fix **API errors and other turn failures not showing in the chat** — the red error panel stayed missing, and sometimes surfaced much later on an unrelated refresh. A failed turn's error travels in a later frame than the snapshot the stream-end flush captured, and since the v0.4.8 sync that stale snapshot was force-pushed with a bumped sequence: it either overwrote the error bubble already on screen or pushed the sequence past the in-flight frame carrying the error, so the webview rejected it. Long conversations hit it more often. The force-push now stands down when a newer frame is already on its way, and an error tail survives a shorter snapshot on the webview side
